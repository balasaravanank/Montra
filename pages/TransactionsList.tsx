...
type DateFilter = 'all' | 'today' | '7days' | 'thisMonth';
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return [...transactions]
      .filter(t => {
        ...
        if (dateFilter === 'today') matchesDate = tDate >= startOfToday;
        else if (dateFilter === '7days') matchesDate = tDate >= startOfLast7Days;
        else if (dateFilter === 'thisMonth') matchesDate = tDate >= startOfThisMonth;
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-24 md:pb-0">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-light text-slate-900 dark:text-white">Transactions</h1>
      ...
      <div className="space-y-3">
        {filteredTransactions.length === 0 ? (
          ...
          filteredTransactions.map((t) => (
            <GlassCard key={t.id} className="p-4 flex items-center justify-between group hover:bg-white/80 dark:hover:bg-white/5 transition-all cursor-default border-white/60 dark:border-white/5">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${CATEGORY_COLORS[t.category]}`}>
                ...
                <div>
                  <h4 className="font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                    ...
                    {t.type === 'income' && t.source && (
                      <span className="text-[10px] bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-1.5 py-0.5 rounded-md font-bold uppercase tracking-wider">
                        {t.source}
                      </span>
                    )}
                  </h4>
              ...
              <div className="flex items-center gap-5">
                <span className={`text-sm font-bold tracking-tight ${t.type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-white'}`}>
                  {t.type === 'income' ? '+' : '-'}{currency}{t.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
                <button 
                  onClick={() => onDelete(t.id)}
                  className="p-2 text-slate-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all opacity-0 group-hover:opacity-100 scale-90"
